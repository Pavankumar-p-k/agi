      ...
      >
        <div className="flex items-center justify-between px-5 h-16 border-b border-[var(--j-border)]">
          <Link href="/" className="flex items-center gap-3">
            <span className="relative h-8 w-8 rotate-45 border border-[var(--j-sky)] shadow-[0_0_18px_rgba(var(--j-sky-rgb),0.18)]">
        ...
        <div className="border-t border-[var(--j-border)] p-4 space-y-4">
          <div>
            <div className="hud-label mb-2 px-1">Theme</div>
            <div className="flex gap-1.5 flex-wrap">
              {THEMES.map((t) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className={`w-7 h-7 text-[10px] font-mono font-medium transition-all ${
                    theme === t.id
          ...
          <div>
            <div className="hud-label mb-1.5 px-1">Font</div>
            <select
              value={font}
              onChange={(e) => setFont(e.target.value as 'sans' | 'mono' | 'display')}
              className="w-full bg-[var(--j-bg)] border border-[var(--j-border)] px-2 py-2 text-xs text-[var(--j-text)] outline-none"
            >
