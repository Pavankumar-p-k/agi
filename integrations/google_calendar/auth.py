...
CREDS_FILE = TOKEN_DIR / "gcal_credentials.json"
...
class GoogleCalendarAuth:
    ...
    def finish_authorization(self, code: str, port: int = 8080) -> bool:
        try:
            ...
            logger.error("[GCalAuth] finish_authorization failed: %s", e)
            return False
    ...
    def authenticate(self, headless: bool = False) -> bool:
        ...
        if not self._creds_path.exists():
            logger.error(
                "[GCalAuth] Credentials file not found at %s. "
                "Download from Google Cloud Console and save as gcal_credentials.json",
                self._creds_path
            )
            return False
        try:
            ...
            logger.error("[GCalAuth] Authentication failed: %s", e)
            return False
    ...
    def _load_and_refresh_token(self) -> bool:
        try:
            ...
            logger.error("[GCalAuth] Token load/refresh failed: %s", e)
            return False
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "healthy": True,
                "calendars_count": len(cal_list.get("items", [])),
...
_auth_instance: GoogleCalendarAuth | None = None
_auth_lock = threading.Lock()


def get_auth() -> GoogleCalendarAuth:
