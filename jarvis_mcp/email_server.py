...
def _resolve_account(selector: str | None) -> dict | None:
    ...
    try:
        ...
        logger.warning("[mcp.email_server] cleanup_email_connection failed: %s", e)
    return None
...
def _load_config(account: str | None = None) -> dict:
    ...
    if row:
        ...
        # Prefer core.secret_storage.decrypt (dev-mode aware). If that fails,
        # fall back to CredentialManager env provider to allow local testing.
        try:
            from core.secret_storage import decrypt as _decrypt
        except Exception as exc:
            logger.warning("[mcp.email_server] core.decrypt import failed: %s", exc)
            _decrypt = lambda v: v  # noqa: E731

        # Helper: try decrypt, otherwise fallback to env via credential manager
        try:
            from core.credentials import default_manager
        except Exception:
            default_manager = None

        def _decrypt_or_fallback(service_name: str, ciphertext: str):
            try:
                return _decrypt(ciphertext)
            except Exception:
                if default_manager:
                    fb = default_manager.get(service_name) or default_manager.get(service_name.upper())
                    if fb:
                        logger.debug("[mcp.email_server] using CredentialManager fallback for %s", service_name)
                        return fb
                # Last resort: return the raw ciphertext so caller can decide
                return ciphertext

        cfg["imap_password"] = _decrypt_or_fallback("IMAP_PASSWORD", row["imap_password"]) if row["imap_password"] else cfg["imap_password"]
        ...
        cfg["smtp_password"] = _decrypt_or_fallback("SMTP_PASSWORD", row["smtp_password"]) if row["smtp_password"] else cfg["smtp_password"]
        ...
        try:
            ...
            logger.warning("[mcp.email_server] fetch_email failed: %s", e)
...
def _detect_sent_folder(conn):
    ...
    try:
        ...
        logger.warning("[mcp.email_server] send_email failed: %s", e)
    return "Sent"
...
def _result_sort_time(result: dict) -> datetime:
    try:
        ...
        logger.warning("[mcp.email_server] handle_webhook failed: %s", e)
    return datetime.min
    ...
    try:
        ...
        if name == "list_emails":
            ...
            try:
                ...
                logger.warning("[mcp.email_server] email_scheduler failed: %s", e)
