...
def _ensure_init():
    ...
    try:
        ...
        logger.warning("[mcp.rag_server] initialize_rag failed: %s", e)

    try:
        from core.constants import PERSONAL_DIR
        from core.personal_docs import PersonalDocsManager
        _personal_docs_manager = PersonalDocsManager(PERSONAL_DIR, _rag_manager)
    except Exception as e:
        logger.warning("[mcp.rag_server] personal_docs_init failed: %s", e)
        ...
        try:
            ...
            if _personal_docs_manager and hasattr(_personal_docs_manager, "add_directory"):
                try:
                    ...
                    logger.warning("[mcp.rag_server] query_rag failed: %s", e)
