...
class MCPServer:
    ...
    async def call_tool(self, name: str, arguments: dict) -> Any:
        ...
        # Governance check — gate dangerous OS actions
        try:
            from governance.GovernanceValidator import GovernanceValidator
            gv = GovernanceValidator()
            task_text = f"{name} {json.dumps(arguments)}"
            gv.validate_execution({"task": task_text})
        except Exception as e:
            logger.warning("[MCP] Governance blocked tool %s: %s", name, e)
            return {
                "content": [{"type": "text", "text": f"Tool '{name}' blocked by governance: {e}"}],
                "isError": True,
            }

        # Durable permission check for OS actions
        try:
            from provider_sdk.permissions import permission_manager, ACTION_PERMISSION_MAP
            required_perm = ACTION_PERMISSION_MAP.get(name)
            if required_perm and not permission_manager.check("mcp_bridge", required_perm):
                permission_manager.log_os_action(
                    action=name,
                    target=json.dumps(arguments)[:500],
                    provider_id="mcp_bridge",
                    permission_required=required_perm,
                    granted=False,
                    reason="Permission denied by durable permission model",
                )
                return {
                    "content": [{"type": "text", "text": f"Tool '{name}' requires permission '{required_perm}' which has not been granted."}],
                    "isError": True,
                }
        except ImportError:
            pass
    ...
    async def handle_websocket(self, websocket: WebSocket):
        ...
        # Token-based authentication — FAIL-CLOSED: reject if auth unavailable
        try:
            from core.gateway.auth import BridgeAuth
            if not await BridgeAuth().authenticate(websocket):
                return
        except ImportError:
            logger.error("[MCP] BridgeAuth not available — rejecting unauthenticated connection")
