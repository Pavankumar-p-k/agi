    ...
    if args.decrypt_test:
        ...
        for key in to_migrate.keys():
            try:
                ...
                if decrypted != to_migrate[key]:
                    # Fallback: check env via credential manager
                    fb = None
                    if default_manager:
                        fb = default_manager.get(key) or default_manager.get(f"{key.upper()}_API_KEY")
                    if fb and fb == to_migrate[key]:
                        print(f"  OK (fallback env): {key}")
                    else:
                ...
                print(f"  FAIL: {key} - {e}")
                return 1
        print("All decryption tests passed or matched env fallback.")
