...
class OfflineAI {
  ...
  Future<AIResponse> process(String input,
    ...
    if (_matches(text, _identityTriggers)) {
      return _buildIdentityReveal();
    }
    if (_matches(text, _reminderTriggers)) {
      return await _handleReminder(input);
    }
    if (_matches(text, _noteTriggers)) {
    ...
    if (text.contains('reminder') &&
        (text.contains('list') ||
            text.contains('show') ||
            text.contains('cheppu') ||
            text.contains('anni') ||
            text.contains('chudandi') ||
            text.contains('em undi'))) {
    ...
    if (text.contains('time') ||
        text.contains('samayam') ||
        text.contains('ippudu entha') ||
        text.contains('ganta entha')) {
    ...
    if ((text.contains('date') ||
      ...
      return AIResponse(
        text:
            'Intha date ${now.day}/${now.month}/${now.year}, ${days[now.weekday % 7]} bro',
        action: AIAction.none,
      );
    }

    // ── Local model handles everything else ──
    final result = await LocalModel.generate(input);
    return AIResponse(
      text: result ?? 'Sorry, I could not generate a response.',
      action: AIAction.none,
      data: const {'intent': 'unknown', 'confidence': 0.0},
    );
  }

  static String? generateReply(String text, String platform) {
    // Simple rule-based fallback for the reply agent
  ...
  Future<AIResponse> _handleNote(String input) async {
    var content = input;
    for (final t in _noteTriggers) {
      content = content.replaceAll(t, '').trim();
    }
    content = content.replaceAll(RegExp(r'^[:—\-\s]+'), '').trim();
    if (content.isEmpty) {
      return const AIResponse(
          text: 'Em note cheyali bro? Cheppu save chestanu',
          action: AIAction.askForContent);
    }
    final n = NoteModel(
    ...
    return AIResponse(
        text: 'Noted bro ✓ Save chesanu',
        action: AIAction.noteSaved,
  ...
  Future<AIResponse> _listReminders() async {
    ...
    if (list.isEmpty) {
      return const AIResponse(
          text: 'Ippudu pending reminders emi ledu bro',
          action: AIAction.listReminders,
          data: {'reminders': []});
    }
    final lines =
        list.take(5).map((r) => '• ${r.title} — ${_formatDT(r.remindAt)}').join('\n');
    return AIResponse(
        text: 'Upcoming reminders:\n$lines',
        action: AIAction.listReminders,
        data: {
          'reminders': list.map((r) => r.toMap()).toList(),
        });
  }
  ...
  Future<AIResponse> _listNotes() async {
    ...
    if (list.isEmpty) {
      return const AIResponse(
          text: 'Emi notes ledu ippudu bro',
          action: AIAction.listNotes,
          data: {'notes': []});
    }
    ...
    return AIResponse(
        ...
        action: AIAction.listNotes,
        data: {
          'notes': list.map((n) => n.toMap()).toList(),
        });
  }
  ...
  DateTime? _parseDateTime(String input) {
    ...
    final now = DateTime.now();
    final minM =
        RegExp(r'(\d+)\s*(?:min|minutes|mins|nimishalu)').firstMatch(text);
    if (minM != null) {
      return now.add(Duration(minutes: int.parse(minM.group(1)!)));
    }
    final hrM = RegExp(r'(\d+)\s*(?:hour|hours|hr|hrs|gantalu)').firstMatch(text);
    if (hrM != null) {
      return now.add(Duration(hours: int.parse(hrM.group(1)!)));
    }
    ...
    if (timeM != null) {
      ...
      if (text.contains('tomorrow') || text.contains('repu')) {
        dt = dt.add(const Duration(days: 1));
      }
      return dt;
    }
    if (text.contains('tomorrow morning') || text.contains('repu morning')) {
      return DateTime(now.year, now.month, now.day + 1, 8, 0);
    }
    if (text.contains('tonight') || text.contains('night')) {
    ...
    for (final e in dayMap.entries) {
      if (text.contains(e.key)) {
        ...
        if (diff <= 0) diff += 7;
        return DateTime(now.year, now.month, now.day + diff, 9, 0);
      }
    }
    return null;
  }
  ...
  String _extractTitle(String input) {
    ...
    for (final trigger in _reminderTriggers) {
      t = t.replaceAll(trigger, '');
    }
  ...
  bool _matches(String text, List<String> triggers) =>
      triggers.any((t) => text.contains(t));
}
...
class AIResponse {
  ...
  const AIResponse(
      {required this.text, this.action = AIAction.none, this.data});
}
