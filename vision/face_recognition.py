...
class FaceRecognizer:
    def __init__(self):
        ...
        self.face_db_path = configuration.get("hardware.faces_dir") or Path("data/faces")
    ...
    def get_embedding(self, image: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            result = DeepFace.represent(
                img_path=image,
                model_name=configuration.get("hardware.face_recognition_model", "VGG-Face"),
                detector_backend=configuration.get("hardware.face_detection_backend", "opencv"),
    ...
    def identify_face(self, image: np.ndarray) -> dict:
        ...
        if best_distance < configuration.get("hardware.face_distance_threshold", 0.38):
