...
class ServiceHealthChecker:
    ...
    async def _check_ollama() -> ServiceHealth:
        ...
        try:
            ...
            base_url = configuration.get("ollama.base_url", "http://localhost:11434").rstrip("/")
            ...
            async with httpx.AsyncClient(timeout=3) as client:
                ...
                if r.status_code == 200:
                    # Exercise the embedding endpoint too: /api/tags alone
                    # does not prove the configured model is usable.
                    embed = await client.post(
                        f"{base_url}/api/embed",
                        json={"model": "nomic-embed-text", "input": ["health"]},
                    )
                    embed_status = getattr(embed, "status_code", 200)
                    if isinstance(embed_status, int) and embed_status >= 400:
                        sh.status = "degraded"
                        sh.error = f"embedding HTTP {embed_status}"
                    else:
                        sh.status = "healthy"
                else:
                    sh.status = "degraded"
        ...
        for mod in ("stt", "tts", "wake_word"):
            try:
                if mod == "stt":
                    ...
                    from assistant.wake_word import get_existing_detector
                    if get_existing_detector() is None:
