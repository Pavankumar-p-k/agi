...
def mock_external_calls(monkeypatch):
    ...
    real_client_req = httpx.Client.request
    real_async_client_req = httpx.AsyncClient.request

    def mock_req(self, *args, **kwargs):
        trans_name = type(getattr(self, "_transport", None)).__name__
        if "TestClient" in type(self).__name__ or "TestClient" in trans_name or "ASGI" in trans_name or "WSGI" in trans_name:
            return real_client_req(self, *args, **kwargs)
        return Mock(status_code=200, text="mocked", json=Mock(return_value={}))

    async def mock_async_req(self, *args, **kwargs):
        trans_name = type(getattr(self, "_transport", None)).__name__
        if "TestClient" in type(self).__name__ or "TestClient" in trans_name or "ASGI" in trans_name or "WSGI" in trans_name:
            return await real_async_client_req(self, *args, **kwargs)
        return Mock(status_code=200, text="mocked", json=Mock(return_value={}))


    monkeypatch.setattr(httpx.Client, "request", mock_req)
    monkeypatch.setattr(httpx.AsyncClient, "request", mock_async_req)
