...
ACTIVITY_EXEMPTIONS = {
    "core/pipeline/stages/activity.py", # pipeline activity lifecycle owner
...
def test_no_direct_activity_mutations(path: Path):
    ...
    assert not violations, (
        ...
        "Activity mutations are confined to the activity owner modules."
    )
...
AUTH_MANAGER_EXEMPTIONS = {
    ...
    "core/routes/admin/router.py", # authenticated admin route dependencies
    "core/routes/auth/router.py",  # authentication endpoints own login lifecycle
...
def test_auth_manager_only_accessed_by_identity_service_and_auth_stage(path: Path):
    ...
    source = _read_source(path)
    # Route modules may depend on FastAPI auth dependencies, but must not
    # access the authentication manager itself.
    if "/routes/" in posix and "AuthManager" not in source and "get_auth_manager" not in source:
        return
...
ACTIVITY_NODE_EXEMPTIONS = {
    ...
    "core/activity/storage.py",  # rehydrates persisted nodes
}
...
WORKER_EXECUTION_EXEMPTIONS = {
    "core/distribution/graph/executor.py",     # graph orchestrator dispatches via Transport
...
def test_research_engines_accessed_only_through_stage_adapters():
    ...
    legacy_exemptions = {
        ...
        "core/routes/agents/research_router.py",
