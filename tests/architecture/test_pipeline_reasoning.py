...
def _fake_provider_router(monkeypatch):
    """Stub the canonical provider_router with a mock ExecutionProvider."""
    ...
    class MockProvider(ExecutionProvider):
        ...
        async def execute(self, task: dict, context=None) -> ExecutionResult:
    ...
    import core.providers.router as router_mod

    fake = MockProvider()
    stub = type("StubRouter", (), {
        "select": lambda *a, **k: fake,
        "select_with_fallback": lambda *a, **k: fake,
    })()
    monkeypatch.setattr(router_mod, "provider_router", stub)
    return fake
...
class TestStepExecutor:
    @pytest.mark.asyncio
    async def test_llm_executor_wraps_provider(self):
    ...
    @pytest.mark.asyncio
    async def test_llm_executor_includes_raw_input(self):
        from core.pipeline.stages.execution import LLMStepExecutor
...
class TestRuntime:
    ...
    async def test_runtime_uses_registered_executor(self):
        from core.pipeline.stages.execution import Runtime, StepExecutor
    ...
    def test_runtime_register_accepts_executor_class(self):
