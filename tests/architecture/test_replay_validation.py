...
from core.pipeline.stages.execution import ExecutionStage, Runtime
...
class FakeProvider(ExecutionProvider):
    ...
    async def execute(self, task: dict, context=None) -> ExecutionResult:
        return ExecutionResult(success=True, output=self._text, metadata={"tokens": 0})
...
class TestRuntimeContextReplay:
    ...
    async def test_execution_runtime_with_fake_services(self):
        ...
        class FakeBus(EventBusProtocol):
