...
class TestSecurityAuditor:
    ...
    def test_audit_auth_dev_mode(self, auditor):
        ...
        with patch.object(configuration, "get", side_effect=lambda key, *a, **k: True if key == "server.dev_mode" else real_get(key, *a, **k)):
    ...
    def test_audit_auth_no_dev_mode(self, auditor):
        from core.configuration import configuration
        real_get = configuration.get
        with patch.object(configuration, "get", side_effect=lambda key, *a, **k: False if key == "server.dev_mode" else real_get(key, *a, **k)):
