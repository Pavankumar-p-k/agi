...
function loadConfig() {
  try {
    ...
    console.warn('[Dot] Config load failed:', e.message);
    if (fs.existsSync(CONFIG_PATH)) {
      try { fs.renameSync(CONFIG_PATH, CONFIG_PATH + '.bak'); } catch {}
    }
  }
  return {};
}
function saveConfig(data) {
  try {
    ...
    console.warn('[Dot] Config save failed:', e.message);
  }
}
...
ipcMain.handle('get-stocks', async (_e, symbol) => {
  try {
    const data = await getJSON(`${JARVIS_URL}/api/dot/stocks?symbol=${encodeURIComponent(symbol || 'AAPL')}`);
    return data;
  } catch {
    return { symbol: symbol || 'AAPL', price: '—', change: null, error: 'Could not reach backend' };
  }
});
...
ipcMain.handle('get-news', async (_e, topic) => {
  try {
    const data = await getJSON(`${JARVIS_URL}/api/dot/news?topic=${encodeURIComponent(topic || 'technology')}`);
    return data;
  } catch {
...
ipcMain.handle('open-app', async (_e, appName) => {
  try {
    if (os.platform() === 'win32') {
      spawn('cmd.exe', ['/c', 'start', '', appName], { shell: false });
    } else {
      spawn(appName, [], { shell: false, detached: true });
    }
    return true;
  } catch { return false; }
});
...
ipcMain.handle('open-terminal', () => {
  try {
    ...
    spawn(cmd, [], { detached: true, shell: false });
    return true;
  } catch { return false; }
});
  ...
  return new Promise((resolve) => {
    try {
      if (process.platform === 'win32') {
        execFile('cmd.exe', ['/s', '/c', command], { shell: false, timeout: 10000 }, (err, stdout, stderr) => {
          resolve({ stdout: stdout || '', stderr: stderr || '', error: err ? err.message : null });
        });
      } else {
        execFile('/bin/sh', ['-c', command], { shell: false, timeout: 10000 }, (err, stdout, stderr) => {
          resolve({ stdout: stdout || '', stderr: stderr || '', error: err ? err.message : null });
        });
      }
    } catch (e) {
...
function launchBackend() {
  ...
  getJSON(`${JARVIS_URL}/api/health`).then(() => {
    ...
    if (app.isPackaged) {
      // Packaged builds don't bundle the Python backend (it's a full project).
      // The user should run the backend server separately or set JARVIS_URL.
      console.warn('[JARVIS Dot] Backend not reachable at ' + JARVIS_URL + '.');
      console.warn('[JARVIS Dot] Please start the JARVIS server manually or set JARVIS_URL to a remote instance.');
    } else {
      // Development: look for jarvis.py in project root
