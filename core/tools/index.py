...
ALWAYS_AVAILABLE = frozenset({
    ...
    "browser_get_history", "browser_get_facts", "browser_research", "browser_list_tabs", "browser_switch_tab", "browser_new_tab", "browser_close_tab",
...
BUILTIN_TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "close_shell": "Close the persistent shell session for the current session ID.",
    "refactor": "Decompose a high-level refactoring goal into steps and execute them. Provide a goal like 'extract email validation to shared module' with optional file list. Can apply specific edits or generate a plan.",
    "undo_edit_file": "Undo the most recent edit to a file by restoring the latest backup.",
    "batch_edit_file": "Edit multiple files at once using a glob pattern. Usage: <glob>\\n<FIND/REPLACE blocks>. All matching files get the same edits.",
    "watch_file": "Watch/tail a file for new content. Use for log files, build output, or any file being written to. Returns new lines since last read.",
    ...
    "create_skill": "Create a new hot-reloadable trigger-based skill with SKILL.md + handler .py. Skill becomes live immediately.",
    ...
    "browser_research": "Run multi-page browser research on a question. Decomposes the question into sub-goals, performs web searches, visits result pages, extracts facts from each page, detects contradictions/agreements, and synthesizes a structured research report. Args (JSON): {\"question\": \"your research question\", \"max_pages\": 5}. Returns sources_consulted, total_facts, summary, agreements, contradictions, gaps, overall_confidence, and recommendations. Use for any question requiring cross-page information gathering and analysis.",
    "scheduler_submit": "Submit a new activity to the autonomous scheduler queue. The scheduler will execute it when its priority and dependencies allow. Args (JSON): {\"goal\": \"Build Android app\", \"priority\": 5, \"node_type\": \"build\"}. Returns activity_id and status. Activities are persisted to SQLite and survive restarts.",
    "scheduler_list": "List all activities in the scheduler queue with their status (pending, running, blocked, completed, failed). Accepts optional status filter. Returns total count, ready count, blocked count, and per-activity details. Use to inspect what the scheduler is working on.",
    "scheduler_status": "Get detailed status of a specific activity by activity_id. Returns priority, score, status, goal, node_type, dependencies, and metadata. Use to check on a submitted activity.",
...
class ToolIndex:
    ...
    def index_mcp_tools(self, mcp_mgr, disabled_map: dict | None = None):
        ...
        try:
            ...
            logger.debug("tools index remove old mcp failed: %s", _e)

        # Get current MCP tools
        try:
            all_tools = mcp_mgr.get_tool_descriptions_for_prompt(disabled_map or {})
        except Exception as _e:
            logger.debug("tools index get mcp tools failed: %s", _e)
            all_tools = ""
