...
def _is_local_endpoint(url: str) -> bool:
    ...
    try:
        ...
        logger.debug("model_context is_local_endpoint failed: %s", _e)
        return False
...
def _query_context_length(endpoint_url: str, model: str) -> int:
    ...
    if _is_local_endpoint(endpoint_url):
        try:
            ...
            logger.debug("model_context query slots failed: %s", _e)
...
def _get_tiktoken_encoder():
    """Lazy-load tiktoken encoder (requires `pip install tiktoken`)."""
    ...
    try:
        ...
        logger.info("[estimate_tokens] using tiktoken cl100k_base")
    except Exception:
        logger.debug("[estimate_tokens] tiktoken not available, using heuristic")
    return _tiktoken_enc
...
def estimate_tokens(messages: List[Dict]) -> int:
    ...
    Falls back to characters * 0.3 + per-message overhead.
    """
            ...
            for item in content:
                if isinstance(item, dict) and item.get("type") == "text":
                    ...
                    if enc:
                        ...
                        total += int(len(text) * 0.3)
        # Add tool calls token overhead
        tc = msg.get("tool_calls") or msg.get("tool_calls")
        if tc:
            total += 12 * len(tc) if isinstance(tc, list) else 12
    return total
