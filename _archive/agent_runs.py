...
def _publish(run: _Run, ev: str) -> None:
    ...
    for q in list(run.subscribers):
        try:
            ...
            logger.debug("publish to subscriber queue failed: %s", _e)
...
async def _drain(session_id: str, agen: AsyncGenerator[str, None],
    ...
    if prev_task is not None and not prev_task.done():
        try:
            ...
            logger.debug("wait for prev_task failed: %s", _e)
    try:
        ...
        try:
            ...
            logger.debug("agen.aclose failed: %s", _e)
        ...
        for q in list(run.subscribers):
            try:
                ...
                logger.debug("publish sentinel to subscriber queue failed: %s", _e)
