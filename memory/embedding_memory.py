...
class EmbeddingMemory:
    ...
    - Honor MEM0_DISABLED to avoid network calls during debugging/testing.
    """
    def __init__(self, db_path: str | None = None):
        ...
        # If MEM0_DISABLED is set, skip embedding network calls
        self.disabled = os.getenv("MEM0_DISABLED", "0").lower() in {"1", "true", "yes"}
        self._init_db()
    ...
    def embed(self, text: str) -> Result:
        # Respect debug flag to avoid external embedding calls
        if self.disabled:
            logger.info("[Memory] MEM0_DISABLED set — skipping embed()")
            return Err(ProviderError("Embedding disabled via MEM0_DISABLED"))
        try:
