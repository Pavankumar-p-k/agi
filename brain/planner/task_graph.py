...
class TaskGraph:
    ...
    def topological_sort(self) -> list[TaskNode]:
        ...
        if len(sorted_nodes) != len(self._nodes):
            logger.warning("[TaskGraph] topological sort incomplete — possible cycle or disconnected nodes")
    ...
    def get_execution_queue(self) -> list[TaskNode]:
        """Return nodes that are ready to execute (all dependencies satisfied)."""
        ready = []
        for node in self._nodes.values():
