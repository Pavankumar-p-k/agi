...
# ── Priority 4: Generalized Failure Memory ──────────────────────

_PATTERN_CACHE: list[tuple[re.Pattern, str, str, dict, int]] = []  # [(compiled_regex, cause, fix_type, params, count)]
...
class FailureMemory:
    ...
    def lookup(self, error_text: str) -> KnownFix | None:
        ...
        for known_sig, fix in self._exact.items():
            if sig.startswith(known_sig) or known_sig.startswith(sig):
                return fix
        # Pattern match, most specific first (fewest wildcards)
        sorted_pats = sorted(
            self._patterns,
        ...
        for pattern, fix in sorted_pats:
            # Match against the ORIGINAL error text (preserves case), not normalized
    ...
    def store(self, error_text: str, cause: str, fix_type: str, fix_params: dict):
        ...
        # Auto-generalize: derive a reusable pattern from this error+fix pair
...
# ── Planner Learning: Architectural Memory ──────────────────────
